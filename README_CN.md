...
- 直接执行代码：`char -e="pln(1+2)"` 或 `char -e "pln(1+2)"`
- 语法检查（不执行）：`char -check script.char` 或 `char -parse script.char`
...
- 使用 `-cmd=` 参数执行代码（支持多行、URL编码和加密，参见下方说明）
...
> **`-e` 与 `-cmd=` 的区别**：`-e` 直接执行代码，不做额外处理，适合简单表达式和单行代码；`-cmd=` 会对代码做特殊字符处理（DealString），并支持配合 `-urlDecode` 参数解码URL编码的多行代码，以及 `//TXDEF#` 前缀的加密代码。
