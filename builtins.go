...
// builtinMakeArrayFunc creates an array with specified length n and fills it with arg.
// If n <= 0, returns arg as is.
// If arg is already an Array, returns a slice of min(n, len(arg)) elements.
// Otherwise, creates a new Array of length n where first element is arg and rest are Undefined.
// This function is for internal use to help with array destructuring.
...
// builtinAppendFunc appends one or more elements to the target collection.
// Supports Array, Bytes, Chars, and Undefined (creates new Array).
// For Array: appends all arguments (c.Args and c.Vargs)
// For Bytes/Chars: appends numeric types (int/uint/char) as bytes/chars
// For Undefined: creates new Array and appends all arguments
...
// builtinRemoveItemsFunc removes items from array between start and end indices (inclusive).
// Parameters: array, start index, end index
// Returns a new array with items in the range [start, end] removed
// start and end must be valid indices within the array bounds
...
// builtinRemoveItemFunc removes a single item from array at specified index.
// Parameters: array, index
// Returns a new array with the item at index removed
// Index must be a valid position within array bounds
...
// builtinAppendListFunc appends all elements from source array/bytes/chars to target.
// Parameters: target, source
// Target can be Array, Bytes, Chars, or Undefined
// Source should be of same type as target (Array for Array, etc.)
// Returns modified target with source elements appended
...
// builtinResetFunc resets a collection to a new size/capacity.
// Parameters: collection, [size], [capacity]
// Supports Array, Bytes, Chars, Map, and String (empties it)
// Returns the reset collection
...
// builtinDeleteFunc deletes a key-value pair from a map.
// Parameters: map (must implement IndexDeleter), key
// Returns error if arg is not a map/syncMap/IndexDeleter
...
// builtinGetArrayItemFunc gets an item from a collection at specified index.
// Parameters: collection, index, [default]
// Returns item at index, or default value if index is out of bounds
// Supports any collection implementing LengthGetter interface
...
// builtinGetMapItemFunc gets a value from a map by key.
// Parameters: map, key, [default]
// Returns value for key, or default value if key doesn't exist
// Returns default if key lookup returns error or undefined
...
// builtinSetMapItemFunc sets a key-value pair in a map.
// Parameters: map, key, value
// Returns Undefined, nil on success
// Returns error if first argument is not a Map
...
// builtinCopyFunc creates a copy of the object if it implements Copier interface.
// Otherwise returns the object as is.
// Returns a new copy of the object or the original if not copiable
...
// builtinRepeatFunc repeats the given object count times.
// Parameters: object (array|string|bytes), count (non-negative integer)
// Returns repeated object, or error if count is negative or type not supported
// Supports Array, String, MutableString, and Bytes types
...
// builtinContainsFunc checks if arg1 is contained in arg0.
// Parameters: collection (map|array|string|bytes), value to check
// For Map/SyncMap: checks if key exists
// For Array: checks if value exists (using Equal comparison)
// For String/MutableString: checks substring containment
// For Bytes: checks if byte sequence exists
// Returns Bool indicating whether value is contained
...
// builtinLenFunc returns the length/size of the given object.
// It supports any object that implements the LengthGetter interface,
// as well as the Any type which wraps other values.
// For LengthGetter objects, it calls the Len() method.
// For Any type, it unwraps the value and calculates the length.
// Returns 0 if the object type does not support length operation.
//
// Examples:
//
//	len("hello")      // returns 5
...
// builtinCapFunc returns the capacity of the given object.
// It supports Array and Bytes types.
// For other types, it returns 0.
// Capacity is the allocated space for the data structure,
// which may be larger than the actual length.
//
// Examples:
//
//	cap([1, 2, 3])           // returns capacity of the array
//	cap(bytes("hello"))      // returns capacity of the bytes slice
...
// builtinSortFunc sorts the given object in ascending order.
// It supports Array, Map, OrderedMap, String, MutableString, and Bytes types.
// For Array, it uses the Less operator to compare elements.
// For Map and OrderedMap, it sorts by string keys.
// For String and MutableString, it sorts by rune values.
// For Bytes, it sorts by byte values.
// Returns the sorted object or an error if sorting fails.
// Returns undefined if the input is undefined or type is not supported.
//
// Examples:
//
//	sort([3, 1, 2])          // returns [1, 2, 3]
//	sort("cba")              // returns "abc"
//	sort(bytes([3, 1, 2]))   // returns bytes([1, 2, 3])
...
// builtinSortReverseFunc sorts the given object in descending (reverse) order.
// It supports Array, Map, OrderedMap, String, MutableString, and Bytes types.
// For Array, it uses the Greater operator to compare elements.
// For Map and OrderedMap, it sorts by string keys in descending order.
// For String and MutableString, it sorts by rune values in descending order.
// For Bytes, it sorts by byte values in descending order.
// Returns the sorted object or an error if sorting fails.
//
// Examples:
//
//	sortReverse([3, 1, 2])     // returns [3, 2, 1]
//	sortReverse("abc")         // returns "cba"
...
// builtinErrorFunc creates an error object with the given message.
// Parameters: message (any type, converted to string)
// Returns an Error object with name "error" and the message
func builtinErrorFunc(arg Object) Object {
	return &Error{Name: "error", Message: arg.String()}
}

// builtinTypeNameFunc returns the type name of the given object.
// Parameters: object (any type)
// Returns the type name as a string (e.g., "int", "string", "Array", etc.)
func builtinTypeNameFunc(arg Object) Object { return ToStringObject(arg.TypeName()) }

// builtinBoolFunc creates a boolean value from the argument.
// Parameters: [value] (optional, any type)
// If no argument provided, returns false
// Otherwise returns true if value is truthy (not falsy), false otherwise
...
// builtinIntFunc creates an integer value from int64.
// Parameters: value (int64)
// Returns an Int object
func builtinIntFunc(v int64) Object { return Int(v) }

// builtinUintFunc creates an unsigned integer value from uint64.
// Parameters: value (uint64)
// Returns a Uint object
func builtinUintFunc(v uint64) Object { return Uint(v) }

// builtinFloatFunc creates a float value from float64.
// Parameters: value (float64)
// Returns a Float object
func builtinFloatFunc(v float64) Object { return Float(v) }

// builtinByteFunc creates a byte value from argument.
// Parameters: [value] (optional, any numeric or string type)
// If no argument provided, returns Byte(0)
// Supports: Bool (1 for true, 0 for false), Byte, Char, Int, Uint, Float, String
// Returns a Byte object, or error if type not supported
...
// builtinCharFunc creates a char/rune value from argument.
// Parameters: value (numeric|string|bool)
// Returns a Char object, or Undefined if conversion fails or value is invalid
// Returns error if type is not supported (numeric, string, bool)
...
// builtinStringFunc creates a string object from the first argument.
...
// builtinMutableStringFunc creates a mutable string and optional member map pairs.
...
// builtinBytesFunc creates a bytes object from arguments or converts a single value to bytes.
...
// builtinBytesWithSizeFunc creates a bytes object with the specified length.
...
// builtinBytesWithCapFunc creates an empty bytes object with the specified capacity.
...
// builtinCharsFunc converts supported values to a chars (rune slice) object.
...
// builtinPrintfFunc prints formatted text to PrintWriter.
...
// char add start
// builtinSystemCmdFunc prints arguments with a trailing newline to PrintWriter.
...
// builtinPrintlnFunc prints arguments with a trailing newline to PrintWriter.
...
// builtinSprintfFunc formats and returns a string using the provided format and arguments.
...
// builtinGlobalsFunc returns the global variables map from the current VM.
func builtinGlobalsFunc(c Call) (Object, error) {
	return c.VM().GetGlobals(), nil
}

// builtinIsErrorFunc checks whether a value is an error, or whether one error matches another.
...
// builtinIsIntFunc reports whether the given object is of Int type.
func builtinIsIntFunc(arg Object) Object {
	_, ok := arg.(Int)
	return Bool(ok)
}

// builtinIsUintFunc reports whether the given object is of Uint type.
func builtinIsUintFunc(arg Object) Object {
	_, ok := arg.(Uint)
	return Bool(ok)
}

// builtinIsFloatFunc reports whether the given object is of Float type.
func builtinIsFloatFunc(arg Object) Object {
	_, ok := arg.(Float)
	return Bool(ok)
}

// builtinIsCharFunc reports whether the given object is of Char type.
func builtinIsCharFunc(arg Object) Object {
	_, ok := arg.(Char)
	return Bool(ok)
}

// builtinIsByteFunc reports whether the given object is of Byte type.
func builtinIsByteFunc(arg Object) Object {
	_, ok := arg.(Byte)
	return Bool(ok)
}

// builtinIsBoolFunc reports whether the given object is of Bool type.
func builtinIsBoolFunc(arg Object) Object {
	_, ok := arg.(Bool)
	return Bool(ok)
}

// builtinIsStringFunc reports whether the given object is of String type.
func builtinIsStringFunc(arg Object) Object {
	_, ok := arg.(String)
	return Bool(ok)
}

// builtinIsBytesFunc reports whether the given object is of Bytes type.
func builtinIsBytesFunc(arg Object) Object {
	_, ok := arg.(Bytes)
	return Bool(ok)
}

// builtinIsCharsFunc reports whether the given object is of Chars type.
func builtinIsCharsFunc(arg Object) Object {
	_, ok := arg.(Chars)
	return Bool(ok)
}

// builtinIsMapFunc reports whether the given object is of Map type.
func builtinIsMapFunc(arg Object) Object {
	_, ok := arg.(Map)
	return Bool(ok)
}

// builtinIsSyncMapFunc reports whether the given object is of SyncMap type.
func builtinIsSyncMapFunc(arg Object) Object {
	_, ok := arg.(*SyncMap)
	return Bool(ok)
}

// builtinIsArrayFunc reports whether the given object is of Array type.
func builtinIsArrayFunc(arg Object) Object {
	_, ok := arg.(Array)
	return Bool(ok)
}

// builtinIsUndefinedFunc reports whether the given object is Undefined.
func builtinIsUndefinedFunc(arg Object) Object {
	_, ok := arg.(*UndefinedType)
	return Bool(ok)
}

// builtinIsFunctionFunc reports whether the given object is any function type.
...
// builtinIsCallableFunc reports whether the object can be called.
func builtinIsCallableFunc(arg Object) Object { return Bool(arg.CanCall()) }

// builtinIsIterableFunc reports whether the object can be iterated.
...
// char add start
// OneResultCallAdapter wraps CallableFunc and returns only one Object result.
...
// toArgsA converts call arguments from offset to []interface{}.
...
// toArgsS converts call arguments from offset to []string.
...
// toArgsN converts call arguments from offset to []int.
...
// like tk.Pass
// fnAR adapts func() to CallableFunc and always returns Undefined.
...
// fnARex adapts func() to CallableExFunc and always returns Undefined.
...
// like tk.GetOSName
// fnARS adapts func() string to CallableFunc returning String.
...
// fnARSex adapts func() string to CallableExFunc returning String.
...
// like time.Now
// fnART adapts func() time.Time to CallableFunc returning Time.
...
// fnARTex adapts func() time.Time to CallableExFunc returning Time.
...
// like tk.RunTasker
// fnARE adapts func() error to CallableFunc returning converted error result.
...
// fnAREex adapts func() error to CallableExFunc returning converted error result.
...
// fnATRS adapts func(time.Time) string to CallableFunc.
...
// fnATRSex adapts func(time.Time) string to CallableExFunc.
...
// like tk.FormatTime
// fnATVsRS adapts func(time.Time, ...string) string to CallableFunc.
...
// fnATVsRSex adapts func(time.Time, ...string) string to CallableExFunc.
...
// like tk.TimeAddSecs
// fnATFRT adapts func(time.Time, float64) time.Time to CallableFunc.
...
// fnATFRTex adapts func(time.Time, float64) time.Time to CallableExFunc.
...
// like tk.TimeAddDate
// fnATIIIRT adapts func(time.Time, int, int, int) time.Time to CallableFunc.
...
// fnATIIIRTex adapts func(time.Time, int, int, int) time.Time to CallableExFunc.
...
// like tk.GetErrStrX
// fnAARS adapts func(interface{}) string to CallableFunc.
...
// fnAARSex adapts func(interface{}) string to CallableExFunc.
...
// like io.Copy
// fnAWtRdRI64E adapts func(io.Writer, io.Reader) (int64, error) to CallableFunc.
...
// fnAWtRdRI64Eex adapts func(io.Writer, io.Reader) (int64, error) to CallableExFunc.
...
// like os.GetEnv
// fnASRS adapts func(string) string to CallableFunc.
...
// fnASRSex adapts func(string) string to CallableExFunc.
...
// fnASSAVaRA adapts func(string, string, interface{}, ...string) interface{} to CallableFunc.
...
// fnASSAVaRAex adapts func(string, string, interface{}, ...string) interface{} to CallableExFunc.
...
// like tk.RuneLen
// fnASRI adapts func(string) int to CallableFunc.
...
// fnASRIex adapts func(string) int to CallableExFunc.
...
// like tk.FromBase64
// fnASRA adapts func(string) interface{} to CallableFunc.
...
// fnASRAex adapts func(string) interface{} to CallableExFunc.
...
// like filepath.Rel
// fnASSRSE adapts func(string, string) (string, error) to CallableFunc.
...
// fnASSRSEex adapts func(string, string) (string, error) to CallableExFunc.
...
// fnASSVsRSE adapts func(string, string, ...string) (string, error) to CallableFunc.
...
// fnASSVsRSEex adapts func(string, string, ...string) (string, error) to CallableExFunc.
...
// fnASRSE adapts func(string) (string, error) to CallableFunc.
...
// fnASRSEex adapts func(string) (string, error) to CallableExFunc.
...
// fnASSRB adapts func(string, string) bool to CallableFunc.
...
// fnASSRBex adapts func(string, string) bool to CallableExFunc.
...
// like tk.SetVar
// fnASAR adapts func(string, interface{}) to CallableFunc.
...
// fnASARex adapts func(string, interface{}) to CallableExFunc.
...
// like tk.DeleteVar
// fnASR adapts func(string) to CallableFunc returning Undefined.
...
// fnASRex adapts func(string) to CallableExFunc returning Undefined.
...
// like math.Sqrt
// fnAFRF adapts func(float64) float64 to CallableFunc.
...
// fnAFRFex adapts func(float64) float64 to CallableExFunc.
...
// fnAFFFFRF adapts func(float64, float64, float64, float64) float64 to CallableFunc.
...
// fnAFFFFRFex adapts func(float64, float64, float64, float64) float64 to CallableExFunc.
...
// fnAFRS adapts func(float64) string to CallableFunc.
...
// fnAFRSex adapts func(float64) string to CallableExFunc.
...
// like tk.HexToBytes
// fnASRLby adapts func(string) []byte to CallableFunc.
...
// fnASRLbyex adapts func(string) []byte to CallableExFunc.
...
// fnASRB adapts func(string) bool to CallableFunc.
...
// fnASRBex adapts func(string) bool to CallableExFunc.
...
// like tk.SetClipText
// fnASRE adapts func(string) error to CallableFunc.
...
// fnASREex adapts func(string) error to CallableExFunc.
...
// fnASSIRS adapts func(string, string, int) string to CallableFunc.
...
// fnASSIRSex adapts func(string, string, int) string to CallableExFunc.
...
// like tk.SplitLines
// fnASRLs adapts func(string) []string to CallableFunc.
...
// fnASRLsex adapts func(string) []string to CallableExFunc.
...
// like tk.GetOSArgs
// fnARLs adapts func() []string to CallableFunc.
...
// fnARLsex adapts func() []string to CallableExFunc.
...
// fnASSRLs adapts func(string, string) []string to CallableFunc.
...
// fnASSRLsex adapts func(string, string) []string to CallableExFunc.
...
// like tk.RegFindAllX
// fnASSIRLs adapts func(string, string, int) []string to CallableFunc.
...
// fnASSIRLsex adapts func(string, string, int) []string to CallableExFunc.
...
// fnASSRA2N adapts func(string, string) [][]int to CallableFunc.
...
// fnASSRA2Nex adapts func(string, string) [][]int to CallableExFunc.
...
// like tk.RegSplitX
// fnASSViRLs adapts func(string, string, ...int) []string to CallableFunc.
...
// fnASSViRLsex adapts func(string, string, ...int) []string to CallableExFunc.
...
// fnASSRLls adapts func(string, string) [][]string to CallableFunc.
...
// fnASSRLlsex adapts func(string, string) [][]string to CallableExFunc.
...
// fnASSRS adapts func(string, string) string to CallableFunc.
...
// fnASSRSex adapts func(string, string) string to CallableExFunc.
...
// fnASSRA adapts func(string, string) interface{} to CallableFunc.
...
// fnASSRAex adapts func(string, string) interface{} to CallableExFunc.
...
// fnASDlRE adapts func(string, delegate) error to CallableFunc.
...
// fnASDlREex adapts func(string, delegate) error to CallableExFunc.
...
// fnARSWRE adapts func(io.Reader, string, io.Writer) error to CallableFunc.
...
// fnARSWREex adapts func(io.Reader, string, io.Writer) error to CallableExFunc.
...
// fnASSRFex adapts func(string, string) float64 to CallableExFunc.
...
// fnASSVIRF adapts func(string, string, ...int) float64 to CallableFunc.
...
// fnASSVIRFex adapts func(string, string, ...int) float64 to CallableExFunc.
...
// like strings.Repeat
// fnASIRS adapts func(string, int) string to CallableFunc.
...
// fnASIRSex adapts func(string, int) string to CallableExFunc.
...
// fnASIRSE adapts func(string, int) (string, error) to CallableFunc.
...
// fnASIRSEex adapts func(string, int) (string, error) to CallableExFunc.
...
// like tk.RegReplaceX
// fnASSSRS adapts func(string, string, string) string to CallableFunc.
...
// fnASSSRSex adapts func(string, string, string) string to CallableExFunc.
...
// fnASSRI adapts func(string, string) int to CallableFunc.
...
// fnASSRIex adapts func(string, string) int to CallableExFunc.
...
// fnASSRA2I adapts func(string, string) [][]int to CallableFunc.
...
// fnASSRA2Iex adapts func(string, string) [][]int to CallableExFunc.
...
// like os.SetEnv
// fnASSRE adapts func(string, string) error to CallableFunc.
...
// fnASSREex adapts func(string, string) error to CallableExFunc.
...
// like filepath.Join
// fnAVsRS adapts variadic string-to-string functions to CallableFunc.
...
// fnAVsRSex adapts variadic string-to-string functions to CallableExFunc.
...
// fnAVsRA adapts variadic string-to-interface functions to CallableFunc.
...
// fnAVsRAex adapts variadic string-to-interface functions to CallableExFunc.
...
// fnAVsRE adapts variadic string-to-error functions to CallableFunc.
...
// fnAVsREex adapts variadic string-to-error functions to CallableExFunc.
...
// like tk.GetSeq
// fnARI adapts func() int to CallableFunc.
...
// fnARIex adapts func() int to CallableExFunc.
...
// like tk.GetChar
// fnARA adapts func() interface{} to CallableFunc.
...
// fnARAex adapts func() interface{} to CallableExFunc.
...
// like tk.SystemCmd
// fnASVsRS adapts func(string, ...string) string to CallableFunc.
...
// fnASVsRSex adapts func(string, ...string) string to CallableExFunc.
...
// like tk.IntToStrX
// fnAAVsRS adapts func(interface{}, ...string) string to CallableFunc.
...
// fnAAVsRSex adapts func(interface{}, ...string) string to CallableExFunc.
...
// fnALyVsRLy adapts func([]byte, ...string) []byte to CallableFunc.
...
// fnALyVsRLyex adapts func([]byte, ...string) []byte to CallableExFunc.
...
// fnALySRLy adapts func([]byte, string) []byte to CallableFunc.
...
// fnALySRLyex adapts func([]byte, string) []byte to CallableExFunc.
...
// fnALyVsRA adapts func([]byte, ...string) interface{} to CallableFunc.
...
// fnALyVsRAex adapts func([]byte, ...string) interface{} to CallableExFunc.
...
// fnALySVsRA adapts func([]byte, string, ...string) interface{} to CallableFunc.
...
// fnALySVsRAex adapts func([]byte, string, ...string) interface{} to CallableExFunc.
...
// fnALyARB adapts func([]byte, interface{}) bool to CallableFunc.
...
// fnALyARBex adapts func([]byte, interface{}) bool to CallableExFunc.
...
// like tk.BytesIndex
// fnALyARI adapts func([]byte, interface{}) int to CallableFunc.
...
// fnALyARIex adapts func([]byte, interface{}) int to CallableExFunc.
...
// fnALyRB adapts func([]byte) bool to CallableFunc.
...
// fnALyRBex adapts func([]byte) bool to CallableExFunc.
...
// like tk.ToPinyin
// fnASVsRA adapts func(string, ...string) interface{} to CallableFunc.
...
// fnASVsRAex adapts func(string, ...string) interface{} to CallableExFunc.
...
// fnASSVsRA adapts func(string, string, ...string) interface{} to CallableFunc.
...
// fnASSVsRAex adapts func(string, string, ...string) interface{} to CallableExFunc.
...
// fnASSSVsRA adapts func(string, string, string, ...string) interface{} to CallableFunc.
...
// fnASSSVsRAex adapts func(string, string, string, ...string) interface{} to CallableExFunc.
...
// like tk.RenameFile
// fnASSVsRE adapts func(string, string, ...string) error to CallableFunc.
...
// fnASSVsREex adapts func(string, string, ...string) error to CallableExFunc.
...
// fnALsSVsRE adapts func([]string, string, ...string) error to CallableFunc.
...
// fnALsSVsREex adapts func([]string, string, ...string) error to CallableExFunc.
...
// like tk.InStrings
// fnASVsRB adapts func(string, ...string) bool to CallableFunc.
...
// fnASVsRBex adapts func(string, ...string) bool to CallableExFunc.
...
// fnASSVsRB adapts func(string, string, ...string) bool to CallableFunc.
...
// fnASSVsRBex adapts func(string, string, ...string) bool to CallableExFunc.
...
// fnASSSVsRS adapts func(string, string, string, ...string) string to CallableFunc.
...
// fnASSSVsRSex adapts func(string, string, string, ...string) string to CallableExFunc.
...
// like tk.GetFileList
// fnASVsRLmss adapts func(string, ...string) []map[string]string to CallableFunc.
...
// fnASVsRLmssex adapts func(string, ...string) []map[string]string to CallableExFunc.
...
// like tk.ErrStrf
// fnASVaRS adapts func(string, ...interface{}) string to CallableFunc.
...
// fnASVaRSex adapts func(string, ...interface{}) string to CallableExFunc.
...
// fnASVaRB adapts func(string, ...interface{}) bool to CallableFunc.
...
// fnASVaRBex adapts func(string, ...interface{}) bool to CallableExFunc.
...
// like fmt.Sprintln
// fnAVaRS adapts variadic interface-to-string functions to CallableFunc.
...
// fnAVaRSex adapts variadic interface-to-string functions to CallableExFunc.
...
// like tk.Errf
// fnASVaRE adapts func(string, ...interface{}) error to CallableFunc.
...
// fnASVaREex adapts func(string, ...interface{}) error to CallableExFunc.
...
// like tk.ErrStrf
// fnAAVaRS adapts func(interface{}, ...interface{}) string to CallableFunc.
...
// fnAAVaRSex adapts func(interface{}, ...interface{}) string to CallableExFunc.
...
// like tk.Compress
// fnAAVaRA adapts func(interface{}, ...interface{}) interface{} to CallableFunc.
...
// fnAAVaRAex adapts func(interface{}, ...interface{}) interface{} to CallableExFunc.
...
// fnASViRA adapts func(string, ...int) interface{} to CallableFunc.
...
// fnASViRAex adapts func(string, ...int) interface{} to CallableExFunc.
...
// fnASViRI adapts func(string, ...int) int to CallableFunc.
...
// fnASViRIex adapts func(string, ...int) int to CallableExFunc.
...
// fnALbySRE adapts func([]byte, string) error to CallableFunc.
...
// fnALbySREex adapts func([]byte, string) error to CallableExFunc.
...
// fnALbyLbyViRLLi adapts func([]byte, []byte, ...int) [][]int to CallableFunc.
...
// fnALbyLbyViRLLiex adapts func([]byte, []byte, ...int) [][]int to CallableExFunc.
...
// fnASSViRLLi adapts func(string, string, ...int) [][]int to CallableFunc.
...
// fnASSViRLLiex adapts func(string, string, ...int) [][]int to CallableExFunc.
...
// fnALsSViRI adapts func([]string, string, ...int) int to CallableFunc.
...
// fnALsSViRIex adapts func([]string, string, ...int) int to CallableExFunc.
...
// fnALsRLs adapts func([]string) []string to CallableFunc.
...
// fnALsRLsex adapts func([]string) []string to CallableExFunc.
...
// like tk.LimitString
// fnASIVsRS adapts func(string, int, ...string) string to CallableFunc.
...
// fnASIVsRSex adapts func(string, int, ...string) string to CallableExFunc.
...
// fnASIIVsRS adapts func(string, int, int, ...string) string to CallableFunc.
...
// fnASIIVsRSex adapts func(string, int, int, ...string) string to CallableExFunc.
...
// like sqltk.ExecDBX
// fnADSVaRA adapts func(*sql.DB, string, ...interface{}) interface{} to CallableFunc.
...
// fnADSVaRAex adapts func(*sql.DB, string, ...interface{}) interface{} to CallableExFunc.
...
// fnADSSVaRA adapts func(*sql.DB, string, string, ...interface{}) interface{} to CallableFunc.
...
// fnADSSVaRAex adapts func(*sql.DB, string, string, ...interface{}) interface{} to CallableExFunc.
...
// like tk.GetWeb
// fnASVaRA adapts func(string, ...interface{}) interface{} to CallableFunc.
...
// fnASVaRAex adapts func(string, ...interface{}) interface{} to CallableExFunc.
...
// like tk.Pln
// fnAVaR adapts variadic interface consumer functions to CallableFunc.
...
// fnAVaRex adapts variadic interface consumer functions to CallableExFunc.
...
// like tk.Min
// fnAVaRA adapts variadic interface-to-interface functions to CallableFunc.
...
// fnAVaRAex adapts variadic interface-to-interface functions to CallableExFunc.
...
// like tk.Ceil
// fnAARA adapts func(interface{}) interface{} to CallableFunc.
...
// fnAARAex adapts func(interface{}) interface{} to CallableExFunc.
...
// like fmt.Print
// fnAVaRIE adapts variadic interface functions returning (int,error) to CallableFunc.
...
// fnAVaRIEex adapts variadic interface functions returning (int,error) to CallableExFunc.
...
// like tk.PlErrX
// fnAAR adapts func(interface{}) to CallableFunc returning Undefined.
...
// fnAARex adapts func(interface{}) to CallableExFunc returning Undefined.
...
// like tk.Pln
// fnASVaR adapts func(string, ...interface{}) to CallableFunc.
...
// fnASVaRex adapts func(string, ...interface{}) to CallableExFunc.
...
// like fmt.Printf
// fnAWSVaRIE adapts func(io.Writer, string, ...interface{}) (int,error) to CallableFunc.
...
// fnAWSVaRIEex adapts func(io.Writer, string, ...interface{}) (int,error) to CallableExFunc.
...
// like fmt.Printf
// fnASVaRIEex adapts func(string, ...interface{}) (int,error) to CallableExFunc.
...
// builtin funcs
// builtinTestByTextFunc asserts two string values are exactly equal for tests.
...
// builtinTestByStartsWithFunc asserts the first string starts with the second.
...
// builtinTestByEndsWithFunc asserts the first string ends with the second.
...
// builtinTestByContainsFunc asserts the first string contains the second.
...
// builtinTestByRegContainsFunc asserts regex pattern appears in the first string.
...
// builtinTestByRegFunc asserts the first string matches the regex pattern.
...
// builtinPassFunc is a no-op builtin that always returns Undefined.
func builtinPassFunc(c Call) (Object, error) {
	return Undefined, nil
}

// builtinPlFunc prints formatted output via tk.Pl using first arg as format string.
...
// builtinTypeCodeFunc returns the numeric type code of the first argument.
...
// builtinGetParamFunc gets positional non-switch argument from an argument array.
...
// builtinGetSwitchFunc gets switch value by prefix from an argument array.
...
// builtinIfSwitchExistsFunc checks whether a switch exists in an argument array.
...
// builtinToJSONFunc converts the first argument to JSON text.
...
// builtinSimpleEncodeFunc encodes text with custom simple encoding.
...
// builtinSimpleDecodeFunc decodes text previously encoded by simple encoding.
...
// builtinBase64EncodeFunc encodes supported input types into base64 string.
...
// builtinBase64EncodeByRawUrlFunc encodes input using raw URL-safe base64.
...
// builtinBase64DecodeByRawUrlFunc decodes raw URL-safe base64 to bytes.
...
// builtinFromJSONFunc parses JSON text into internal objects.
...
// builtinFromXmlFunc parses XML text into internal objects.
...
// builtinEncodeSimpleMapFunc encodes a map[string]string-like Map to text.
...
// builtinDecodeSimpleMapFunc decodes simple-map text into Map object.
...
// builtinIsErrXFunc checks whether argument represents an error value.
...
// isErrX reports whether object is an error-like value.
...
// builtinGetNowTimeStampFunc returns current timestamp string.
func builtinGetNowTimeStampFunc(c Call) (Object, error) {
	return String(tk.GetTimeStampMid(time.Now())), nil
}

// builtinRunTickerFunc runs a periodic ticker callback with given interval seconds.
...
// builtinTimeBeforeFunc reports whether the first time is before the second.
...
// builtinIsNilFunc checks whether the argument is nil-like.
...
// builtinIsNilOrEmptyFunc checks whether argument is nil-like or empty.
...
// builtinIsNilOrErrFunc checks whether argument is nil-like or error-like.
...
// builtinSleepFunc sleeps for given duration seconds string/number.
...
// builtinLockFunc acquires a mutex lock or numbered lock.
...
// builtinUnlockFunc releases a mutex lock or numbered lock.
...
// builtinRLockFunc acquires read lock on a mutex.
...
// builtinRUnlockFunc releases read lock on a mutex.
...
// builtinTryLockFunc attempts to lock mutex without blocking.
...
// builtinTryRLockFunc attempts to read-lock mutex without blocking.
...
// builtinExitFunc exits process with optional status code.
...
// builtinTimeFunc creates Time from input or returns current time.
...
// builtinToTimeFunc converts input to Time object.
...
// builtinCallNamedFuncFunc invokes a registered named function.
...
// builtinCallInternalFuncFunc invokes a function object reflectively.
...
// builtinGetNamedValueFunc returns a registered named value.
...
// builtinNewExFunc creates a new reflected object from arguments.
...
// builtinCallMethodExFunc calls a method by name on an object.
...
// builtinGetValueFunc returns the underlying value of the first argument.
...
// builtinGetMemberFunc gets a named member from an object.
...
// builtinSetMemberFunc sets a named member on an object.
...
// builtinSetValueFunc sets value for objects implementing ValueSetter.
...
// builtinCallMethodFunc invokes an object's method with given arguments.
...
// builtinRegQuoteFunc escapes a string for regex literal usage.
...
// builtinGetClipTextFunc returns clipboard text content.
...
// builtinAnyFunc wraps a value into Any while preserving original type metadata.
...
// builtinSetValueByRefFunc writes a value through supported reference wrappers.
...
// builtinUnrefFunc dereferences supported reference wrappers to a value.
...
// builtinDumpVarFunc dumps the first argument for debugging.
...
// builtinDebugInfoFunc returns VM bytecode and runtime debug information.
...
// builtinToStrFunc converts the first argument to string.
...
// builtinToHexFunc converts the first argument to hexadecimal text.
...
// builtinToIntFunc converts the first argument to int with optional default.
...
// builtinToBoolFunc converts the first argument to bool.
...
// builtinToBoolWithDefaultFunc converts value to bool with optional default.
...
// builtinToFloatFunc converts the first argument to float with optional default.
...
// builtinTypeOfAnyFunc returns Go type string of Any.Value.
...
// builtinPltFunc prints the value(s) with their Charlang type name(s).
// It is a debugging helper function that displays both the type and value
// of each argument passed to it.
//
// Output format:
//   - No arguments: prints a newline
//   - Single argument: prints "(TypeName)value" followed by newline
//   - Multiple arguments: prints "[index] (TypeName)value" for each argument
//
// Returns the number of bytes printed or an error if the print operation fails.
//
// Examples:
//
//	plt("hello")           // prints: (string)hello
//	plt(42)                // prints: (int)42
//	plt(1, "a", true)      // prints: [0] (int)1
//	                       //         [1] (string)a
//	                       //         [2] (bool)true
...
// builtinGetEnvFunc returns environment variable value by name.
...
// builtinCheckErrXFunc exits the process if argument is error-like.
...
// builtinCheckEmptyFunc exits process when value is undefined or empty string.
...
// builtinErrToEmptyFunc converts error-like values to empty string.
...
// builtinTrimFunc trims spaces/chars from a value converted to string.
...
// builtinTrimErrFunc trims string unless the input is error-like.
...
// builtinStrTrimFunc trims a String value without undefined-to-empty conversion.
...
// builtinStrTrimStartFunc removes a prefix string from the start.
...
// builtinStrTrimEndFunc removes a suffix string from the end.
...
// builtinStrTrimLeftFunc trims cutset chars from the left side.
...
// builtinStrTrimRightFunc trims cutset chars from the right side.
...
// builtinRemoveDirFunc removes a directory, optionally recursively.
...
// builtinGetFileInfoFunc returns file metadata map for path or File.
...
// builtinRemovePathFunc removes file or directory, optionally recursively.
...
// builtinSetStdinFunc redirects process stdin to the provided File.
...
// builtinGetMultiLineInputFunc reads multiline input with optional callback/options.
...
// builtinSetStdoutFunc redirects process stdout to the provided File.
...
// builtinSetStderrFunc redirects process stderr to the provided File.
...
// builtinGetPipeFunc creates an OS pipe and returns [reader, writer].
...
// builtinGetWebBytesFunc fetches URL content and returns bytes.
...
// builtinGetWebRespBodyFunc fetches URL response body and returns stream/data.
...
// builtinGetWebBytesWithHeadersFunc downloads web bytes with request headers/options.
...
// builtinParseUrlFunc parses URL string into structured object.
...
// builtinParseQueryFunc parses URL query string to map/object.
...
// builtinOpenFileFunc opens a file path with options and returns File.
...
// builtinLoadBytesFromFileLimitFunc reads up to limit bytes from file path.
...
// builtinCloseFileFunc closes a File object.
...
// builtinArchiveFilesToZipFunc archives files into a zip file.
...
// builtinReadCsvFunc reads CSV from reader or file and returns rows.
...
// builtinExcelOpenFunc opens an Excel workbook from reader/bytes/path.
...
// builtinExcelOpenFromBytesFunc opens an Excel workbook from bytes.
...
// builtinExcelOpenFileFunc opens an Excel workbook from file path.
...
// builtinExcelSaveAsFunc saves an Excel workbook to a new file path.
...
// builtinExcelWriteToFunc writes workbook data to response or writer.
...
// builtinExcelWriteToBytesFunc serializes workbook to bytes.
...
// builtinExcelCloseFunc closes an opened Excel workbook.
...
// builtinExcelReadSheetFunc reads all rows from one worksheet.
...
// builtinExcelReadAllFunc reads all sheets and rows from workbook.
...
// builtinExcelGetSheetNameFunc gets worksheet name by index.
...
// builtinExcelGetSheetCountFunc returns number of worksheets.
...
// builtinExcelGetSheetListFunc returns worksheet name list.
...
// builtinExcelReadCellFunc reads a single cell value from worksheet.
...
// builtinExcelReadCellImagesFunc reads images anchored at a worksheet cell.
...
// builtinExcelWriteSheetFunc writes 2D data rows into a worksheet.
...
// builtinExcelWriteCellFunc writes one cell value in a worksheet.
...
// builtinExcelGetColumnIndexByNameFunc converts column name to numeric index.
...
// builtinExcelGetColumnNameByIndexFunc converts numeric index to column name.
...
// builtinExcelNewSheetFunc creates a new worksheet and returns its index.
...
// builtinExcelRemoveSheetFunc deletes a worksheet by name.
...
// builtinWriteCsvFunc writes CSV records to writer or file path.
...
// builtinFtpCreateDirFunc creates a remote directory on FTP server.
...
// builtinFtpListFunc lists directory entries on FTP server.
...
// builtinFtpSizeFunc gets remote file size from FTP server.
...
// builtinFtpUploadFunc uploads a local file to FTP server.
...
// builtinFtpDownloadBytesFunc downloads a remote FTP file as bytes.
...
// builtinFtpDownloadFileFunc downloads a remote FTP file to local path.
...
// builtinFtpGetReaderFunc returns streaming reader for remote FTP file.
...
// builtinFtpRemoveFileFunc deletes a remote file on FTP server.
...
// builtinFtpUploadFromReaderFunc uploads content from reader to FTP server.
...
// builtinSshUploadFunc uploads a local file to remote path over SSH.
...
// builtinSshUploadBytesFunc uploads in-memory bytes to remote path over SSH.
...
// builtinSshDownloadFunc downloads remote file to local path over SSH.
...
// builtinSshDownloadBytesFunc downloads remote file content as bytes over SSH.
...
// builtinSshListFunc lists remote directory entries over SSH.
...
// builtinSshIfFileExistsFunc checks whether a remote path exists over SSH.
...
// builtinSshGetFileInfoFunc gets metadata of remote path over SSH.
...
// builtinSshRunFunc runs a remote shell command over SSH.
...
// builtinSshRenameFunc renames or moves a remote path over SSH.
...
// builtinSshJoinPathFunc joins remote path segments using server semantics.
...
// builtinSshRemoveFileFunc removes a remote file over SSH.
...
// builtinSshRemoveDirFunc removes a remote directory over SSH.
...
// builtinSshEnsureMakeDirsFunc ensures remote directories exist over SSH.
...
// builtinStringBuilderFunc creates a StringBuilder with optional initial text.
...
// builtinStatusResultFunc creates a StatusResult from status/value/objects.
...
// builtinSeqFunc creates a sequence object from arguments.
...
// builtinMutexFunc creates a mutex object from arguments.
...
// builtinCharCodeFunc compiles source text into CharCode object.
...
// builtinGelFunc creates a Gel object from arguments.
...
// builtinOrderedMapFunc creates an ordered map object.
...
// builtinExcelFunc creates an Excel object.
func builtinExcelFunc(c Call) (Object, error) {
	return NewExcel(c)
}

// builtinBigIntFunc creates a BigInt object.
func builtinBigIntFunc(c Call) (Object, error) {
	return NewBigInt(c)
}

// builtinBigFloatFunc creates a BigFloat object.
// builtinBigFloatFunc creates a BigFloat object.
func builtinBigFloatFunc(c Call) (Object, error) {
	return NewBigFloat(c)
}

// builtinMuxFunc creates a Mux object.
// builtinMuxFunc creates a Mux object.
...
// builtinWriteRespFunc writes bytes/string content to response or writer.
// builtinWriteRespFunc writes content to HTTP response or writer.
...
// builtinServeFileFunc serves a file through HTTP response.
...
// builtinIsEncryptedFunc checks whether input bytes/string is TXDEF-encrypted.
// builtinIsEncryptedFunc checks whether bytes/string is encrypted by TXDEF.
...
// builtinGenJwtTokenFunc generates JWT token from payload and secret.
// builtinGenJwtTokenFunc generates JWT token with payload and secret.
...
// builtinParseJwtTokenFunc parses and validates JWT token with secret.
// builtinParseJwtTokenFunc parses and validates JWT token.
...
// builtinParseReqFormFunc parses HTTP request form fields.
// builtinParseReqFormFunc parses URL-encoded form values from request.
...
// builtinParseReqFormExFunc parses normal and multipart request forms.
// builtinParseReqFormExFunc parses normal and multipart form values.
...
// builtinParseReqMultipartFormFunc parses multipart request form only.
// builtinParseReqMultipartFormFunc parses multipart form values only.
...
// builtinSetRespHeaderFunc sets an HTTP response header key/value.
// builtinSetRespHeaderFunc sets HTTP response header key/value.
...
// builtinGetReqHeaderFunc gets one header value from HTTP request.
// builtinGetReqHeaderFunc gets one request header value by key.
...
// builtinGetReqHeadersFunc gets all headers from HTTP request.
// builtinGetReqHeadersFunc gets all request headers as map.
...
// builtinIsHttpsFunc reports whether HTTP request uses HTTPS.
// builtinIsHttpsFunc checks whether request uses HTTPS.
...
// builtinPrepareMultiPartFieldFromBytesFunc creates multipart field payload from bytes.
// builtinPrepareMultiPartFieldFromBytesFunc builds multipart payload for one field.
...
// builtinPrepareMultiPartFileFromBytesFunc creates multipart file payload from bytes.
// builtinPrepareMultiPartFileFromBytesFunc builds multipart payload for one file.
...
// builtinPostRequestFunc sends HTTP POST request with text/bytes body.
// builtinPostRequestFunc sends HTTP POST request with text or bytes body.
...
// builtinHttpRedirectFunc issues an HTTP redirect response.
// builtinHttpRedirectFunc sends HTTP redirect response.
...
// builtinGetReqBodyFunc reads and returns HTTP request body bytes.
// builtinGetReqBodyFunc reads request body and returns bytes.
...
// builtinNewFunc creates a new object via builtinMakeFunc.
...
// builtinMakeFunc constructs a new object by type name and arguments.
...
// builtinReadAllStrFunc reads all content from reader-like input as string.
...
// builtinReadAllBytesFunc reads all content from reader-like input as bytes.
...
// builtinReadBytesFunc reads up to N bytes from reader-like input.
...
// builtinWriteStrFunc writes string content to writer-like targets.
...
// builtinWriteBytesFunc writes bytes/string content to writer-like targets.
...
// builtinWriteBytesAtFunc copies bytes into a Bytes buffer at index.
...
// builtinCopyBytesFunc copies bytes from source Bytes to destination Bytes.
...
// builtinDatabaseFunc opens a database connection from driver and DSN.
...
// builtinFatalfFunc prints an error message and exits the program with status code 1.
// It accepts a format string and optional arguments, similar to printf.
// This function is used for fatal error handling during script execution.
// The program will terminate immediately after printing the error message.
//
// Parameters:
//   - format: A format string (same as fmt.Sprintf)
//   - args: Optional arguments for the format string
//
// Examples:
//
//	fatalf("File not found: %v", filename)
//	fatalf("Invalid parameter: %s", paramName)
...
// builtinGenJSONRespFunc generates a JSON response for HTTP requests.
// It creates a JSONP (JSON with Padding) response with status and value fields.
// This function is commonly used in microservice development for returning
// standardized API responses.
//
// Parameters:
//   - request: The HTTP request object (*HttpReq)
//   - status: Response status (e.g., "success", "fail")
//   - value: Response value/data (will be converted to string)
//   - options: Optional additional parameters
//
// Returns:
//
//	A JSON string in the format: {"Status":"status","Value":"value"}
//
// Examples:
//
//	genResp(requestG, "success", "operation completed")
//	genResp(requestG, "fail", "invalid input")
...
// BuiltinDbCloseFunc closes a database connection.
// It should be called after completing database operations to release resources.
// Failing to close database connections may lead to resource leaks.
//
// Parameters:
...
// builtinLeClearFunc clears the line editor buffer.
// It resets the internal line buffer to an empty state.
// This function is used when working with multi-line text editing.
//
// Parameters:
//   - VM context (implicit)
//
// Examples:
//
//	leClear()  // Clears the current line editor buffer
...
// builtinLeLoadFromStrFunc loads text into the line editor buffer from a string.
// It splits the input string by lines and stores them in the editor buffer.
// Previous content in the buffer will be replaced.
//
// Parameters:
//   - text: The text string to load into the editor
//
// Examples:
//
//	leLoadFromStr("Line 1\nLine 2\nLine 3")  // Loads 3 lines into editor
...
// builtinLeAppendFromStrFunc appends text to the line editor buffer from a string.
// It splits the input string by lines and appends them to the existing buffer.
// Unlike leLoadFromStr, this function preserves existing content.
//
// Parameters:
//   - text: The text string to append to the editor
//
// Examples:
//
//	leAppendFromStr("Line 4\nLine 5")  // Appends 2 lines to current buffer
...
// builtinLeSaveToStrFunc saves the line editor buffer content to a string.
// It joins all lines in the buffer with newline characters.
// Returns the complete text content as a single string.
//
// Returns:
//
//	The content of the line editor buffer as a string.
//
// Examples:
//
//	content := leSaveToStr()  // Gets all lines as a single string
...
// builtinLeLoadFromFileFunc loads text from a file into the line editor buffer.
// It reads the file content, splits it by lines, and stores in the editor buffer.
// Supports optional line limit parameter to load only a specific number of lines.
//
// Parameters:
...
// Examples:
//
//	leLoadFromFile("document.txt")       // Load entire file
//	leLoadFromFile("document.txt", 100)  // Load first 100 lines
...
// builtinLeLoadFromUrlFunc loads text from a URL into the line editor buffer.
// It fetches the content from the specified URL and splits it by lines.
// Supports additional options for HTTP request configuration.
//
// Parameters:
//   - url: The URL to fetch content from
//   - options: (Optional) Additional HTTP request options
//
// Examples:
//
//	leLoadFromUrl("https://example.com/file.txt")
...
// builtinLeSshInfoFunc returns the current SSH connection information for the line editor.
// This is used when working with remote files via SSH.
//
// Returns:
//
//	A string representation of the SSH connection info map.
//
// Examples:
//
//	info := leSshInfo()  // Get current SSH connection settings
...
// builtinLeInfoFunc returns information about the current line editor buffer.
// It provides statistics including line count, character count, and rune count.
// This is useful for understanding the size and complexity of the loaded text.
//
// Returns:
//
//	A formatted string containing buffer statistics.
//
// Examples:
//
//	info := leInfo()  // Returns: "line count: 10, char count: 256, rune count: 240"
...
// builtinLeLoadFromSshFunc loads text from a remote file via SSH into the line editor buffer.
// It connects to the SSH server, reads the file content, and splits it by lines.
// SSH connection parameters can be specified or retrieved from existing SSH info.
//
// Parameters:
...
//   - options: (Optional) Additional SSH options (-host=, -port=, -user=, -password=, -path=)
//
// Examples:
//
//	leLoadFromSsh("192.168.1.1", "22", "user", "password", "/path/to/file.txt")
...
// builtinLeSaveToSshFunc saves the line editor buffer content to a remote file via SSH.
// It connects to the SSH server and writes the buffer content to the specified file.
// SSH connection parameters are retrieved from existing SSH info or can be overridden.
//
// Parameters:
//   - options: (Optional) SSH options (-host=, -port=, -user=, -password=, -path=)
//
// Examples:
//
//	leSaveToSsh()  // Save to previously connected SSH path
//	leSaveToSsh("-path=/new/path/file.txt")  // Save to different path
...
// builtinLeAppendFromFileFunc appends content from a file to the line editor buffer.
// It reads the file content, splits it by lines, and appends to the existing buffer.
// Unlike leLoadFromFile, this function preserves existing content in the buffer.
//
// Parameters:
//   - filePath: Path to the file to append
//   - limit: (Optional) Maximum number of lines to append
//
// Examples:
//
//	leAppendFromFile("additional.txt")       // Append entire file
//	leAppendFromFile("additional.txt", 50)   // Append first 50 lines
...
// builtinLeSaveToFileFunc saves the line editor buffer content to a file.
// It joins all lines in the buffer with newline characters and writes to the specified file.
...
// Examples:
//
//	leSaveToFile("output.txt")  // Save buffer to file
...
// builtinLeAppendToFileFunc appends the line editor buffer content to a file.
// It joins all lines in the buffer and appends them to the end of the specified file.
// Unlike leSaveToFile, this function preserves existing file content.
//
// Parameters:
...
// Examples:
//
//	leAppendToFile("log.txt")  // Append buffer content to file
...
// builtinLeLoadFromClipFunc loads text from the system clipboard into the line editor buffer.
// It retrieves the clipboard content, splits it by lines, and stores in the editor buffer.
// Previous content in the buffer will be replaced.
//
// Examples:
//
//	leLoadFromClip()  // Load clipboard content into editor
...
// builtinLeSaveToClipFunc saves the line editor buffer content to the system clipboard.
// It joins all lines in the buffer and sets the result as the clipboard text.
// This allows easy copying of editor content to other applications.
//
// Examples:
//
//	leSaveToClip()  // Copy buffer content to clipboard
...
// builtinLeViewAllFunc displays all lines in the line editor buffer.
// It prints each line with its line number (unless -nl option is specified).
// This function is useful for viewing the complete content of the editor.
//
// Parameters:
//   - options: (Optional) -nl to hide line numbers
//
// Examples:
//
//	leViewAll()      // View all lines with line numbers
//	leViewAll("-nl") // View all lines without line numbers
...
// builtinLeViewLinesFunc displays a range of lines from the line editor buffer.
// It prints lines from the start index to the end index (inclusive).
// Supports negative indices to count from the end of the buffer.
//
// Parameters:
//   - start: Start line index (0-based, negative for from end)
//   - end: End line index (0-based, negative for from end, -1 for last line)
//
// Examples:
//
//	leViewLines(0, 10)    // View first 11 lines (0-10)
//	leViewLines(-5, -1)   // View last 5 lines
...
// builtinLeGetListFunc returns the line editor buffer as an array of strings.
// Each element in the returned array represents one line from the buffer.
// This function is useful for programmatic access to the editor content.
//
// Returns:
//
//	An array containing all lines in the editor buffer.
//
// Examples:
//
//	lines := leGetList()  // Get all lines as array
...
// builtinLeViewLineFunc displays a specific line from the line editor buffer.
// It prints the line content with its line number.
//
// Parameters:
//   - lineIndex: The index of the line to view (0-based)
//
// Examples:
//
//	leViewLine(0)    // View first line
//	leViewLine(10)   // View 11th line
...
// builtinLeSortFunc sorts line-editor buffer in ascending/descending order.
...
// builtinLeConvertToUtf8Func converts entire line-editor buffer to UTF-8.
...
// builtinStrToUtf8Func converts input string to UTF-8 using encoding option.
...
// builtinBytesGbToUtf8StrFunc converts encoded bytes to UTF-8 string.
...
// builtinIsUtf8Func checks whether bytes/string content is valid UTF-8.
...
// builtinSimpleStrToMapFunc parses simple "k:v|k2:v2" text to map.
...
// builtinSimpleStrToMapReverseFunc parses simple text with key/value reversed.
...
// builtinReverseMapFunc swaps keys and values of a Map.
...
// builtinLeGetLineFunc returns one line from line-editor buffer by index.
...
// builtinLeSetLineFunc replaces one line in line-editor buffer by index.
...
// builtinLeSetLinesFunc replaces a range of lines in line-editor buffer.
...
// builtinLeInsertLineFunc inserts one or more lines at index.
...
// builtinLeAppendLineFunc appends one or more lines to line-editor buffer.
...
// builtinLeRemoveLineFunc removes one line from line-editor buffer by index.
...
// builtinLeRemoveLinesFunc removes a range of lines from line-editor buffer.
...
// builtinLeFindLinesFunc finds lines matching regex in line-editor buffer.
...
// builtinLeFindFunc finds first regex match and returns its position info.
...
// builtinLeFindAllFunc finds all regex matches in line-editor buffer.
...
// builtinLeReplaceFunc replaces regex matches across line-editor buffer.
...
// builtinLePrintFunc prints line-editor buffer with optional metadata.
...
// builtinAwsSignFunc signs AWS request parameters with secret key.
...
// builtinMagicFunc loads magic script by id and returns executable Gel.
...
// builtinS3PutObjectFunc uploads object data to S3-compatible storage.
...
// builtinS3GetObjectUrlFunc generates presigned download URL for object.
...
// builtinS3GetObjectTagsFunc retrieves object tags from S3 storage.
...
// builtinS3GetObjectStatFunc retrieves object stat metadata from S3.
...
// builtinS3GetObjectBytesFunc downloads object content as bytes from S3.
...
// builtinS3GetObjectTextFunc downloads object content as text from S3.
...
// builtinS3GetObjectReaderFunc opens streaming reader for S3 object.
...
// builtinS3GetObjectToFileFunc downloads S3 object to local file path.
...
// builtinS3StatObjectFunc returns S3 object stat info, optionally compact.
...
// builtinS3RemoveObjectFunc removes object from S3-compatible storage.
...
// builtinS3CopyObjectFunc copies an object between S3 buckets/paths.
...
// builtinS3MoveObjectFunc moves object by copy-then-delete in S3 storage.
...
// builtinS3ListObjectsFunc lists objects in S3 bucket with filters/options.
...
// builtinReplaceHtmlByMapFunc replaces template placeholders in HTML string.
...
// builtinDocxReplacePatternFunc replaces placeholder patterns in DOCX bytes.
...
// builtinDocxGetPlaceholdersFunc extracts placeholders from DOCX bytes.
...
// builtinShowTableFunc renders a 2D array as formatted text table.
...
// builtinDocxToStrsFunc converts DOCX input to extracted text lines.
...
// builtinSendMailFunc sends email with recipients, body, and attachments.
...
// builtinGuiServerCommandFunc sends command request to GUI server endpoint.
...
// builtinArrayContainsFunc checks whether array/any contains target value.
...
// builtinGetRandomIntFunc returns random int, optionally within range.
...
// builtinGetRandomFloatFunc returns random float in [0,1).
...
// builtinGetRandomStrFunc generates random string with optional settings.
...
// builtintStrUnquoteFunc unquotes a quoted string literal.
...
// builtinSscanfFunc scans formatted data from string into targets.
...
// builtinBitNotFunc applies bitwise-not (or bool negation) to value.
...
// builtinToOrderedMapFunc converts Map to OrderedMap object.
...
// builtinAdjustFloatFunc normalizes float precision for Float/BigFloat.
...
// builtinStrSplitNFunc splits string by separator with required limit.
...
// builtinStrJoinFunc joins array/list elements into one string.
...
// builtinMathSqrtFunc returns square root of numeric input.
...
// builtinMathExpFunc returns e**x for numeric input.
...
// builtinMathLogFunc returns natural logarithm for numeric input.
...
// builtinMathLog10Func returns base-10 logarithm for numeric input.
...
// builtinMathSinFunc returns sine of numeric input (radians).
...
// builtinMathCosFunc returns cosine of numeric input (radians).
...
// builtinMathTanFunc returns tangent of numeric input (radians).
...
// builtinMathAtan2Func returns atan2(y, x) for two numeric inputs.
...
// builtinMathPiFunc returns mathematical constant Pi.
func builtinMathPiFunc(c Call) (Object, error) {
	return Float(math.Pi), nil
}

// builtinMathEFunc returns mathematical constant E.
func builtinMathEFunc(c Call) (Object, error) {
	return Float(math.E), nil
}

// builtinMathPowFunc returns x^y for numeric inputs.
...
// builtinMathAbsFunc returns absolute value of numeric input.
...
// builtinWriteRespHeaderFunc writes HTTP response status/header metadata.
...
// builtinImageToAsciiFunc converts an image to ASCII-art lines.
...
// builtinGetImageInfoFunc returns width and height of image.
...
// builtinStrToRgbaFunc parses color string and returns RGBA components.
...
// builtinEncodeImageFunc encodes image to writer in selected format.
...
// builtinEncodeBytesInImageFunc embeds bytes payload into image data.
...
// builtinDecodeBytesFromImageFunc extracts embedded bytes from image.
...
// builtinDrawImageOnImageFunc draws one image onto another image.
...
// builtinDrawTextWrappedOnImageFunc draws wrapped text onto image.
...
// builtinAddWatermarkToImageFunc applies watermark image onto base image.
...
// builtinSetImageOpacityFunc adjusts opacity of an image.
...
// builtinGenQrFunc generates QR code image from text.
...
// builtinScanQrFunc scans QR code text from image.
...
// builtinAesEncryptFunc encrypts bytes/string with AES (ECB/CBC modes).
...
// builtinAesDecryptFunc decrypts AES-encrypted bytes/string data.
...
// SHA1 returns SHA-1 digest bytes for input data.
...
// bytes2Hex converts bytes to lowercase hexadecimal string.
...
// getSHA256 returns SHA-256 hex digest of a string.
...
// newRSAEncryption creates RSAEncryption from a base64 public key.
...
// decryptBASE64 decodes a base64 string into raw bytes.
...
// generatePublicKey parses DER bytes into an RSA public key.
...
// Encrypt encrypts plaintext using RSA-OAEP and returns URL-escaped base64 text.
...
// encryptBlock encrypts one data block with RSA-OAEP.
...
// builtinGetSha256WithKeyYYFunc computes SHA256-related keyed digest value.
...
// builtinRsaEncryptStrYYFunc encrypts text using provided RSA public key.
...
// builtinSaveImageToFileFunc saves Image object to file path.
...
// builtinSaveImageToBytesFunc encodes Image object and returns bytes.
...
// builtinResizeImageFunc resizes image to target width/height.
...
// builtinPlotDataToStrFunc renders numeric series as ASCII plot string.
...
// builtinPlotLoadFontFunc loads chart font from file and optional default.
...
// builtinPlotDataToImageFunc renders numeric series into chart image/bytes.
...
// builtinCloseFunc closes supported resource objects safely.
...
// builtinSortArrayFunc sorts array/string/bytes with optional key/order.
