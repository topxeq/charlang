...
func TestModuleStrings(t *testing.T) {
	...
	require.NoError(t, err)
	require.EqualValues(t, true, ret)
	ret, err = equalFold.Call(String("CHAR"), String("chars"))
	require.NoError(t, err)
	require.EqualValues(t, false, ret)
	...
	require.NoError(t, err)
	require.EqualValues(t, "abc", ret.(String))
	...
	require.NoError(t, err)
	require.EqualValues(t, "======", ret.(String))
	...
	require.NoError(t, err)
	require.EqualValues(t, "abcabcabc", ret.(String))
	...
	require.NoError(t, err)
	require.EqualValues(t, "a(bc)def(bc)", ret.(String))
	ret, err = replace.Call(
		String("abcdefbc"), String("bc"), String("(bc)"), Int(1))
	require.NoError(t, err)
	require.EqualValues(t, "a(bc)defbc", ret.(String))

	split := Module["Split"]
	ret, err = split.Call(String("abc;def;"), String(";"))
	require.NoError(t, err)
	require.Equal(t, 3, len(ret.(Array)))
	require.EqualValues(t, "abc", ret.(Array)[0].(String))
	require.EqualValues(t, "def", ret.(Array)[1].(String))
	require.EqualValues(t, "", ret.(Array)[2].(String))
	...
	require.EqualValues(t, "abc;def;", ret.(Array)[0].(String))
	...
	require.NoError(t, err)
	require.EqualValues(t, "Хлеб", ret.(String))

	toLower := Module["ToLower"]
	ret, err = toLower.Call(String("ÇİĞÖŞÜ"))
	require.NoError(t, err)
	require.EqualValues(t, "çiğöşü", ret.(String))
	...
	require.NoError(t, err)
	require.EqualValues(t, "def", ret.(String))
	...
	require.NoError(t, err)
	require.EqualValues(t, "abcdef", ret.(String))
