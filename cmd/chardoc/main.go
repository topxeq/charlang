// chardoc reads a go package, which must be a charlang stdlib module, extracts and
// groups package comments to create the charlang module documentation.
//
// usage: ./chardoc <source dir> <output file>
//
// Examples:
//
// go run ./cmd/chardoc ./stdlib/time ./docs/stdlib-time.md
//
// go run ./cmd/chardoc ./stdlib/json -
package main
...
//		moduleMap = charlangtime.Module
	...
	case "fmt":
		moduleMap = charlangfmt.Module
	case "json":
		moduleMap = charlangjson.Module
	default:
...
func extractComment(cgrp *ast.CommentGroup) (string, bool) {
	...
	if strings.HasPrefix(p0, chardocPrefix) {
		return parts[1], true
	}
	return "", false
}
...
func writeToFile(pkgs map[string]*ast.Package, outFile string) error {
	...
	_, err = fmt.Fprintf(f, "\n[//]: <> (Generated by chardoc. DO NOT EDIT.)\n\n")
	if err != nil {
		_ = f.Close()
